    ...
    <div :class="`offcanvas offcanvas-end`" data-bs-scroll="true" tabindex="-1" id="booking-form" aria-labelledby="offcanvasBookingForm">
      ...
      <template v-else-if="SINLGE_STEP == 'MAIN' && status != 'checkout'">
        ...
        <div class="offcanvas-body border-top">
          <div class="form-group" v-if="bookingType !== 'CALENDER_BOOKING' && business.options.length > 1">
            <Multiselect id="business_id" placeholder="Select Business" v-model="business_id" :disabled="is_paid || filterStatus(status).is_disabled" :value="business_id" v-bind="singleSelectOption" :options="business.options" @select="businessSelect" @change="removeBusiness" class="form-group"></Multiselect>
          </div>
          <div class="form-group" v-if="bookingType !== 'CALENDER_BOOKING' && business_id">
              ...
              <div v-for="packages in userPackage" class="col-md-6">
                <div v-if="packages.payment_status === 1" class="card bg-soft-secondary">
                  <div class="card-body text-gray">
                    <div class="d-flex justify-content-between mb-3">
                      <div>
                        <small class="bg-soft-secondary rounded-pill py-2 px-4">{{ packages.business_name }}</small>
                      </div>
...
import { BRANCH_LIST, IS_HOLIDAY } from '@/vue/constants/business'
    ...
    if (value.id !== null && value.id !== undefined && value.id !== 0) {
      ...
      getRequest({ url: EDIT_URL, id: id.value }).then((res) => {
        if (res.status) {
          ...
          businessSelect(res.data.business_id)
      ...
      business_id.value = value.business_id
...
const defaultData = () => {
  ...
  return {
    id: null,
    business_id: props.bookingData.business_id || null,
    note: '',
    start_date_time: null,
...
useOnOffcanvasShow('booking-form', () => {
  ...
  business_id.value = props.bookingData.business_id
  getCustomers()
  businessSelect(business_id.value)
  getProducts()
  getpackages(business_id.value)
})
...
const getSlots = () => {
  listingRequest({ url: SLOT_LIST, data: { business_id: business_id.value, date: current_date.value } }).then((res) => {
    if (res.status) {
      const now = new Date()
...
// On Select
const businessSelect = (value) => {
  useSelect({ url: EMPLOYEE_LIST, data: { business_id: value } }, { value: 'id', label: 'name' }).then((data) => (employee.value = data))
...
function fetchHolidays(value) {
  listingRequest({ url: IS_HOLIDAY, data: { business_id: value } }).then((data) => {
    console.log(data)
...
const employeeSelect = (value) => {
  useSelect({ url: SERVICE_LIST, data: { id: value, business_id: business_id.value } }, { value: 'service_id', label: 'service_name' }).then((data) => (service.value = data))
}
...
const getpackages = (business_id) => {
  useSelect({ url: PACKAGE_LIST, data: { business_id: business_id } }, { value: 'id', label: 'name' }).then((data) => {
    packages.value = data
  })
}
...
const getUserPackages = (user_id) => {
  getRequest({ url: USER_PACKAGE_LIST, id: user_id }).then((data) => {
    if (business_id.value) {
      userPackage.value = data.filter((pkg) => pkg.business_id === business_id.value)
    } else {
