      ...
      <div class="offcanvas-body">
        <div class="row">
          <div class="col-12 row">
            <div class="col-md-8">
              <div class="row">
                <InputField class="col-md-6" :is-required="true" :label="$t('employee.lbl_first_name')" :placeholder="$t('employee.first_name')" v-model="first_name" :error-message="errors['first_name']" :error-messages="errorMessages['first_name']"></InputField>
                <InputField class="col-md-6" :is-required="true" :label="$t('employee.lbl_last_name')" :placeholder="$t('employee.last_name')" v-model="last_name" :error-message="errors['last_name']" :error-messages="errorMessages['last_name']" autocomplete="off"> </InputField>

                <InputField class="col-md-6" :is-required="true" :label="$t('employee.lbl_Email')" :placeholder="$t('employee.email_address')" v-model="email" :error-message="errors['email']" :error-messages="errorMessages['email']"></InputField>
            ...
            <div class="row" v-if="currentId === 0">
              <InputField type="password" class="col-md-6" :is-required="true" :label="$t('employee.lbl_password')" :placeholder="$t('employee.password')" v-model="password" :error-message="errors['password']" :autocomplete="newpassword" :error-messages="errorMessages['password']"></InputField>

              <InputField type="password" class="col-md-6" :is-required="true" :label="$t('employee.lbl_confirm_password')" :placeholder="$t('employee.confirm_password')" v-model="confirm_password" :error-message="errors['confirm_password']" :error-messages="errorMessages['confirm_password']"></InputField>
            </div>
            ...
            <div class="form-group m-0 col-md-4">
              <div class="form-check">
                <input class="form-check-input" type="checkbox" :true-value="1" :false-value="0" v-model="is_manager" id="is-manager" :checked="is_manager" />
                <label class="form-check-label" for="is-manager">
                  {{ $t('employee.lbl_is_manager') }}
                </label>
              </div>
            </div>
            <div class="form-group col-md-12" v-if="business.options.length > 1 && selectedSessionBusinessId == ''">
              <label class="form-label" for="business">{{ $t('employee.lbl_select_business') }}</label
              ><span class="text-danger">*</span>
              <Multiselect id="business_id" v-model="business_id" :value="business_id" placeholder="Select Business" v-bind="singleSelectOption" :options="business.options" @select="businessSelect" class="form-group"> </Multiselect>
              <span v-if="errorMessages['business_id']">
                <ul class="text-danger">
                  <li v-for="err in errorMessages['business_id']" :key="err">{{ err }}</li>
                </ul>
              </span>
              <span class="text-danger">{{ errors.business_id }}</span>
            </div>
            ...
            <div class="form-group col-md-12">
              ...
              <Multiselect id="commission_id" v-model="commission_id" :value="commission_id" placeholder="Select Commission" v-bind="singleSelectOption" :options="commissions.options" class="form-group"></Multiselect>
            ...
            <InputField class="col-md-6" :label="$t('employee.lbl_about_self')" :placeholder="$t('employee.about_self')" v-model="about_self" :error-message="errors['about_self']" :error-messages="errorMessages['about_self']"></InputField>
            <InputField class="col-md-6" :label="$t('employee.lbl_expert')" :placeholder="$t('employee.expert')" v-model="expert" :error-message="errors['expert']" :error-messages="errorMessages['expert']"></InputField>
            <InputField class="col-md-6" :label="$t('employee.lbl_facebook_link')" :placeholder="$t('employee.facebook_link')" v-model="facebook_link" :error-message="errors['facebook_link']" :error-messages="errorMessages['facebook_link']"></InputField>
            <InputField class="col-md-6" :label="$t('employee.lbl_instagram_link')" :placeholder="$t('employee.instagram_link')" v-model="instagram_link" :error-message="errors['instagram_link']" :error-messages="errorMessages['instagram_link']"></InputField>
            <InputField class="col-md-6" :label="$t('employee.lbl_twitter_link')" :placeholder="$t('employee.Twitter_link')" v-model="twitter_link" :error-message="errors['twitter_link']" :error-messages="errorMessages['twitter_link']"></InputField>
            <InputField class="col-md-6" :label="$t('employee.lbl_dribbble_link')" :placeholder="$t('employee.dribble_link')" v-model="dribbble_link" :error-message="errors['dribbble_link']" :error-messages="errorMessages['dribbble_link']"></InputField>
            <div class="form-group">
...
const props = defineProps({
  ...
  selectedSessionBusinessId: { type: Number, default: null }
})
...
const currentId = useModuleId(() => {
  useSelect({ url: BRANCH_LIST }, { value: 'id', label: 'name' }).then((data) => {
    business.value = data
    if (props.selectedSessionBusinessId !== null) {
      business_id.value = props.selectedSessionBusinessId
    } else if (data.options.length === 1) {
      business_id.value = data.options[0].value
      businessSelect()
    }
    businessSelect()
  })
...
const business = ref({ options: [], list: [] })
...
const businessSelect = () => {
  useSelect({ url: SERVICE_LIST, data: { business_id: business_id.value } }, { value: 'id', label: 'name' }).then((data) => (services.value = data))
}
...
const validationSchema = yup.object({
  ...
  commission_id: yup.string().required('Select commission is a required field'),
  business_id: yup.string().required('Select Business is a required field')
})
