...
use Modules\Employee\Models\BranchEmployee; // Updated import
    ...
    public function run()
    {
        ...
        if (env('IS_DUMMY_DATA')) {
            foreach ($employee as $key => $employee_data) {
                ...
                if ($emp->is_manager) {
                    ...
                    $businessId = fake()->numberBetween(1, 5);
                }
                BranchEmployee::create([ // Updated reference
                ...
                if ($emp->is_manager) {
                    ...
                    Business::where('id', $businessId)->update(['manager_id' => $emp->id]);
                } else {
        ...
        if (env('IS_FAKE_DATA')) {
            User::factory()->count(19)->create()->each(function ($emp) {
                ...
                BranchEmployee::create([ // Updated reference
                    'business_id' => fake()->numberBetween(1, 5),
                    'employee_id' => $emp->id,
                ]);
                EmployeeCommission::create([
                    'employee_id' => $emp->id,
