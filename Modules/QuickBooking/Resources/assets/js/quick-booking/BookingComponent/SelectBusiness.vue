<template>
  <div class="card-list-data">
    <div class="row row-cols-1 row-cols-md-2" v-if="!IS_LOADER">
      <div class="col" v-for="(item, index) in businessList" :key="item">
        <div class="iq-widget">
          <input type="radio" :id="item.name + item.id" name="radio" :value="item.id" v-model="business_id" class="btn-check" @change="onChange" />
          <label :for="item.name + item.id" class="d-block w-100">
            <div class="card iq-business-box">
              <div class="card-body">
                <div class="text-center mb-4">
                  <div class="business-image">
                    <img :src="item.feature_image" class="avatar-70 rounded-circle" alt="feature-image" loading="lazy" />
                  </div>
                  <div class="business-content mt-3 mt-sm-0">
                    <h4 class="mb-1">{{ item.name || 'No Business' }}</h4>
    ...
    <div v-else class="row row-cols-1 row-cols-md-2">
      <div class="col" v-for="index in 4" :key="index">
        <div class="iq-widget card card-skeleton">
          <div class="card-body">
            <div class="text-center mb-4">
              <div class="skeleton skeleton-image avatar-70 m-auto rounded-circle"></div>
    ...
    <div class="h-100 w-75 d-flex align-items-center justify-content-center" v-if="businessList.length == 0 && !IS_LOADER">We apologize, but currently, there are no available businesses for booking appointments.</div>
  </div>
  <div class="card-footer">
    ...
    <button type="button" class="btn btn-primary iq-text-uppercase" :disabled="business_id !== null ? false : true" v-if="wizardNext" @click="nextTabChange(wizardNext)">{{ $t('quick_booking.lbl_next') }}</button>
  </div>
</template>
<script setup>
// Library Import
...
watch(
  () => business_id.value,
  (value) => {
    store.updateBookingValues({ key: 'business_id', value: value })
  },
  { deep: true }
)
watch(
...
.iq-business-box:hover {
  border-color: var(--bs-primary);
  transform: translateY(-5px);
}

.iq-business-box::after {
  position: absolute;
  content: '';
...
.iq-widget .btn-check:checked + label .iq-business-box {
  border-color: var(--bs-primary);
  background: rgba(var(--bs-primary-rgb), 0.1);
}

.iq-widget .btn-check:checked + label .iq-business-box::after {
  opacity: 1;
}

.iq-business-box .card-body {
  padding: 20px;
}
.iq-contact-detail p {
  font-size: 14px;
