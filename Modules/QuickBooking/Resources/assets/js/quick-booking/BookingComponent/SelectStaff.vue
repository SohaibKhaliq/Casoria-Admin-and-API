    ...
    <div v-else class="row row-cols-1 row-cols-md-2 row-cols-lg-3">
      <div class="col" v-for="index in 6" :key="index">
        <div class="iq-widget card iq-card card-skeleton">
          <div class="card-body text-center p-0">
            <div class="skeleton skeleton-image avatar-90 rounded-circle m-auto"></div>
            <h5 class="skeleton skeleton-title w-10 mt-3 mb-0"></h5>
          </div>
        </div>
      </div>
    </div>
    <div class="h-100 w-75 d-flex align-items-center justify-content-center mx-auto" v-if="employeeList.length == 0">We apologize for the inconvenience, but currently, there are no staff members available at this particular salon business.</div>
  </div>
...
const getStaffs = () => {
  ...
  const data = {
    business_id: store.booking.business_id,
    service_id: store.booking.services[0].service_id,
    start_date_time: store.booking.services[0].start_date_time
  }
  if (data.business_id !== null || data.service_id !== null || data.start_date_time) {
