...
const getSlots = () => {
  ...
  listingRequest({
    url: SLOT_TIME_LIST,
    data: { business_id: store.booking.business_id, date: date.value, employee_id: store.booking.employee_id, service_id: store.booking.services[0].service_id }
  }).then((res) => {
    setTimeout(() => {
...
const getHolidaySlots = () => {
  ...
  listingRequest({
    ...
    data: { business_id: store.booking.business_id, employee_id: store.booking.employee_id, service_id: store.booking.services[0].service_id }
  }).then((res) => {
    setTimeout(() => {
      IS_LOADER.value = false
...
const updateHolidayConfig = () => {
  const disabledDays = holidaySlotList.value.filter((slot) => slot.is_holiday === 1).map((slot) => slot.day.toLowerCase())
...
onMounted(() => {
  if (store.booking.business_id && store.booking.employee_id && store.booking.service_id) {
    getHolidaySlots()
  }
})

watch(
  () => store.booking.business_id,
  (newBusinessId) => {
    if (newBusinessId && store.booking.employee_id) {
      getHolidaySlots()
    }
  }
)
...
watch(
  () => store.booking.employee_id,
  (newEmployeeId) => {
    if (store.booking.business_id && newEmployeeId) {
      getHolidaySlots()
    }
  }
)
...
watch(
  () => store.booking.services[0].service_id,
  (serviceId) => {
