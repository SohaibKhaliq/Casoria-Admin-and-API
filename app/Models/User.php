...
class User extends Authenticatable implements HasMedia, MustVerifyEmail
{
    ...
    public function scopeBusiness($query)
    {
        ...
        if (isset($business_id)) {
            $query->join('business_employee', 'users.id', '=', 'business_employee.employee_id')
                ->where('business_employee.business_id', $business_id);
        }
    }
    ...
    public function scopeBookingEmployeesList($query)
    {
        return $query->select('users.*')
            ->active()
            ->varified()
            ->calenderResource()->employee()->business()->orderBy('id', 'ASC');
    }
