<template>
  <form @submit="formSubmit" class="bussiness-hour">
    ...
    <div class="form-group col-12" v-if="business.options.length > 1">
      <label class="form-label">{{ $t('setting_business_hours.lbl_name') }}<span class="text-danger">*</span></label>
      <Multiselect id="business_id" v-model="business_id" v-bind="singleSelectOption" :options="business.options" @select="businessSelect" class="form-group"></Multiselect>
...
const businessSelect = (e) => {
  const businessId = business_id.value

  getRequest({ url: LISTING_URL, id: { business_id: businessId } }).then((res) => {
    if (res.status) {
      if (res.data != '') {
...
const formSubmit = handleSubmit((values) => {
  ...
  values.business_id = business_id.value
