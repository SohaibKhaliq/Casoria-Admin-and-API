<template>
  <form @submit="formSubmit">
    ...
    <div class="row mb-2">
      <div class="col-12">
        <div class="form-group" v-if="business.options.length > 1">
          <label class="form-label">{{ $t('business.select_business') }}<span class="text-danger">*</span> </label>
          <Multiselect id="business_id" v-model="business_id" placeholder="Select Business" v-bind="singleSelectOption" :options="business.options" @select="businessSelect" class="form-group"></Multiselect>
...
import { BRANCH_LIST } from '@/vue/constants/business'
...
const fetchData = () => {
  getRequest({ url: LISTING_URL, id: { business_id: business_id.value } }).then((res) => {
    if (res.status) {
