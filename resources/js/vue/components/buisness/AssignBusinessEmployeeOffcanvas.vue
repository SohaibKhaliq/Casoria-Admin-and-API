  ...
  <form @submit.prevent="formSubmit">
    <div class="offcanvas offcanvas-end" tabindex="-1" id="staff-assign-form" aria-labelledby="form-offcanvasLabel">
      <div class="offcanvas-header">
        <h4 v-if="business_name">{{ business_name }}</h4>
      </div>
      <div class="offcanvas-body">
        <div class="d-flex flex-column">
          <div class="mb-4">
            <Multiselect v-model="assign_ids" :placeholder="$t('business.select_staff')" :value="assign_ids" :canClear="false" v-bind="staffs" @select="selectEmployee" id="employees_ids">
...
import { POST_ASSIGN_URL, GET_ASSIGN_URL } from '../../constants/business'
import { EMPLOYEE_LIST } from '../../constants/business'
...
const businessId = useModuleId(() => {
  getRequest({ url: GET_ASSIGN_URL, id: businessId.value }).then((res) => {
    if (res.status && res.data) {
      selectedEmployee.value = res.data
      business_name.value = selectedEmployee.value[0].business_name
...
const formSubmit = () => {
  ...
  if (hasUpdates) {
    confirmcancleSwal({
      title: 'Do you want to make sure that if the staff is in another business, they will be moved here?'
    }).then((result) => {
      if (result.isConfirmed) {
        updateRequest({ url: POST_ASSIGN_URL, id: businessId.value, body: data }).then((res) => reset_close_offcanvas(res))
      }
    })
  } else {
    renderedDataTable.ajax.reload(null, false)
